...
void setup()
{
  ...
  //Console.begin(115200); //115200 baud, 8 data bits, 1 stop bit, no parity, XON/XOFF flow control
  Console.println("");
...
void loop()
{
  if (Console.available() > 0) {
    Console.println("Press 's' to change speed, 'b' to brake, 'u' to unbrake, and 'c' to read current");
    char set = Console.read();
    if (set == 's') {
      float temp = Console.parseFloat();
      md.setSpeed2(temp, temp);
      delay(1000);
    }
    else if (set == 'b') {
      Console.println("Braking...");
      md.setBrake2();
      delay(1000);
    }
    else if (set == 'u') {
      Console.println("Unbraking...");
      md.clearBrake2();
      delay(1000);
    }
    else if (set == 'c') {
      Console.println("Reading...");
      Console.println(md.getCurrent(MOTOR_1));
      Console.println(md.getCurrent(MOTOR_2));
      delay(1000);
    }
    delay(2000);
  }
